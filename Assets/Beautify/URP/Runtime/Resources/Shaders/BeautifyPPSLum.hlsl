...
#define BEAUTIFY_PPSLUM_FX
    ...
    float3    _AFData;
    	...
    	TEXTURE2D_X(_BloomSourceTex, sampler_BloomSourceTex);
	    TEXTURE2D_X(_BloomSourceDepth, sampler_BloomSourceDepth);
	...
	float4 FragLum (VaryingsLum i) : SV_Target {
		...
		#if BEAUTIFY_BLOOM_USE_LAYER
		    float4 c = SAMPLE_TEXTURE2D_X(_BloomSourceTex, sampler_LinearClamp, i.uv);
		#else
		...
		#if BEAUTIFY_BLOOM_USE_LAYER
			float depth02 = Linear01Depth(UNITY_SAMPLE_DEPTH(tex2D(_BloomSourceDepth, i.uv)));
	...
	VaryingsCrossLum VertCrossLum(Attributes v) {
   	...
   	float4 FragLumAntiflicker(VaryingsCrossLum i) : SV_Target {
		...
		#if BEAUTIFY_BLOOM_USE_LAYER
			float4 c1 = SAMPLE_TEXTURE2D_X(_BloomSourceTex, sampler_LinearClamp, uv1);
			float4 c2 = SAMPLE_TEXTURE2D_X(_BloomSourceTex, sampler_LinearClamp, uv2);
			float4 c3 = SAMPLE_TEXTURE2D_X(_BloomSourceTex, sampler_LinearClamp, uv3);
			float4 c4 = SAMPLE_TEXTURE2D_X(_BloomSourceTex, sampler_LinearClamp, uv4);
		#else
